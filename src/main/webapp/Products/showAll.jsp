...
<html lang="es">
	<head>
		...
		<meta
			name="keywords"
			content="Codo a Codo - Full Stack Java, Back-end, HTML, Java, MySQL, Práctica, Supermercado, Tabla de productos"
		/>
		<link
		...
		/>
		<title>Supermercado - Tabla de Productos</title>
	</head>
	<body class="min-vh-100 d-flex flex-column bg-secondary bg-opacity-50">
		...
		<jsp:include page="../components/navbar.jsp">
			...
			<jsp:param name="searchAction" value="/products/searchByBrand" />
			<jsp:param name="searchValue" value="<%= searchValue %>" />
			<jsp:param name="searchName" value="brand" />
			<jsp:param name="searchPlaceholder" value="Ingrese la marca..." />
		</jsp:include>
		...
		<nav class="mx-2 mt-4">
  			<ul class="breadcrumb">
    			...
    			<li class="breadcrumb-item active" aria-current="page">Ver tabla de productos</li>
  			</ul>
		</nav>
		...
		<main class="mb-4">
			...
			%>
				...
				%>
						<div class="alert alert-warning mx-3 d-flex align-items-center justify-content-start gap-1" role="alert">
							<svg class="bi bi-info-circle" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
			  				...
			  				<span>Se han encontrado <%= products.size() %> resultados que coinciden con la marca '<%= searchValue %>'.</span>
						</div>
				<%
					};
				%>
									...
									<tr>
										...
										<td>
											<div class="d-flex align-items-center justify-content-center gap-3">
												<button class="btn btn-sm btn-danger px-4 d-flex align-items-center gap-1" type="button" onclick="location.href = '<%= request.getContextPath() %>/products/delete?ID=<%= ID %>';">
												...
												<button class="btn btn-sm btn-info px-4 d-flex align-items-center gap-1" type="button" onclick="location.href = '<%= request.getContextPath() %>/products/update?ID=<%= ID %>';">
				...
				<div class="mx-2 mt-3 d-flex align-items-stretch justify-content-center gap-5">
					<button class="btn btn-outline-success w-25 d-flex align-items-center justify-content-center gap-1" type="button" onclick="location.href = '<%= request.getContextPath() %>/products/add.jsp';">
					...
					<button class="btn btn-outline-danger w-25 d-flex align-items-center justify-content-center gap-1" type="button" onclick="location.href = '<%= request.getContextPath() %>/products/deleteAll';">
			...
			%>
				<div class="alert alert-danger mx-3 d-flex align-items-center justify-content-start gap-1" role="alert">
					<svg class="bi bi-exclamation-triangle" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
			  		...
			  		<span>No se han encontrado resultados que coinciden con la marca '<%= searchValue %>'.</span>
				</div>
				<div class="mx-2 mt-3 d-flex align-items-center justify-content-center gap-5">
					<button class="w-25 btn btn-light d-flex align-items-center gap-1 justify-content-center" type="button" onclick="location.href = '<%= request.getContextPath() %>/products/all';">
